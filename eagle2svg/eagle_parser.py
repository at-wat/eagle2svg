...
class Symbol(object):
    def __init__(self, data):
        ...
        self.pins = []
        if 'wire' in data:
            for wire_data in eagle_list(data['wire']):
                self.wires.append(Wire(wire_data))
        if 'circle' in data:
            for circle_data in eagle_list(data['circle']):
                self.circles.append(Circle(circle_data))
        if 'pad' in data:
            for pad_data in eagle_list(data['pad']):
                self.pads.append(Pad(pad_data))
        if 'rectangle' in data:
            for rectangle_data in eagle_list(data['rectangle']):
                self.rectangles.append(Rectangle(rectangle_data))
        if 'polygon' in data:
            for polygon_data in eagle_list(data['polygon']):
                self.polygons.append(Polygon(polygon_data))
        if 'text' in data:
            for text_data in eagle_list(data['text']):
                self.texts.append(Text(text_data))
        if 'frame' in data:
            for frame_data in eagle_list(data['frame']):
                self.frames.append(Frame(frame_data))
        if 'pin' in data:
            for pin_data in eagle_list(data['pin']):
...
class Device(object):
    def __init__(self, data):
        ...
        self.connects = {}
        if 'connects' in data:
            for connect_data in eagle_named_list(data['connects']):
...
class Deviceset(object):
    def __init__(self, data):
        ...
        self.gates = {}
        self.devices = {}
        for gate_data in eagle_named_list(data['gates']):
            gate = Gate(gate_data)
            self.gates[gate.name] = gate
        for device_data in eagle_named_list(data['devices']):
...
class Library(object):
    def __init__(self, data):
        ...
        self.symbols = {}
        self.devicesets = {}
        for symbol_data in eagle_named_list(data['symbols']):
            symbol = Symbol(symbol_data)
            self.symbols[symbol.name] = symbol
        for deviceset_data in eagle_named_list(data['devicesets']):
...
class Instance(object):
    def __init__(self, data):
        ...
        if 'attribute' in data:
            for attribute_data in eagle_list(data['attribute']):
    ...
    def __init__(self, data):
        ...
        if 'wire' in data:
            for wire_data in eagle_list(data['wire']):
                self.wires.append(Wire(wire_data))
        if 'junction' in data:
            for junction_data in eagle_list(data['junction']):
                self.junctions.append(Junction(junction_data))
        if 'label' in data:
            for label_data in eagle_list(data['label']):
...
class Net(object):
    def __init__(self, data):
        ...
        self.segments = []
        for segment_data in eagle_list(data['segment']):
...
class Sheet(object):
    def __init__(self, data):
        ...
        self.busses = []
        self.nets = []
        for plain_data in eagle_named_list(data['plain']):
            self.plains.append(Plain(plain_data))
        for instance_data in eagle_named_list(data['instances']):
            self.instances.append(Instance(instance_data))
        for bus_data in eagle_named_list(data['busses']):
            self.busses.append(Bus(bus_data))
        for net_data in eagle_named_list(data['nets']):
...
class EagleFileBase(object):
    def __init__(self, data):
        self.parts = {}
        for part_data in eagle_named_list(data['parts']):
            part = Part(part_data)
...
class Schematic(EagleFileBase):
    def __init__(self, data):
        ...
        for sheet_data in eagle_named_list(data['sheets']):
            self.sheets.append(Sheet(sheet_data))

        for library_data in eagle_named_list(data['libraries']):
